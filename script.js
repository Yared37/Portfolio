...
function setupNavigation() {
    ...
    const circularNavButtons = document.querySelectorAll('.circular-nav-btn');
    ...
    // Smooth scroll for circular navigation buttons
    circularNavButtons.forEach(button => {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            */
            ...
            // You'll need to add: <script src="https://cdn.jsdelivr.net/npm/@emailjs/browser@3/dist/email.min.js"></script>
            // And initialize: emailjs.init('YOUR_PUBLIC_KEY');
            /*
            ...
            */
            ...
            // Option 3: Current implementation - Opens email client
            // This works immediately without any backend setup
            const subject = encodeURIComponent(`Contact from Portfolio: ${data.name}`);
            const body = encodeURIComponent(`Hello Yared,\n\n${data.message}\n\n---\nFrom: ${data.name}\nEmail: ${data.email}`);
            const mailtoLink = `mailto:yaredatrsaw357@gmail.com?subject=${subject}&body=${body}`;
            
            // Show success message immediately
            showFormMessage('Opening your email client... If it doesn\'t open, please email me directly at yaredatrsaw357@gmail.com', 'success');
            ...
            showFormMessage('Sorry, there was an error. Please email me directly at yaredatrsaw357@gmail.com', 'error');
            submitBtn.disabled = false;
            submitText.textContent = 'Send Message';
        }
    });
...
// Add smooth scrolling for anchor links (excluding nav links which are handled separately)
document.addEventListener('click', function(e) {
    if (e.target.matches('a[href^="#"]') && 
        !e.target.classList.contains('nav-link-vertical') && 
        !e.target.classList.contains('circular-nav-btn')) {
